...
const Request: React.FC = () => {
    ...
    const onSubmit = async (data: FormData) => {
        ...
        const expiryInSeconds = parseInt(data.expirationDays) * 86400 + parseInt(data.expirationHours) * 3600;

        try {
            let bitcoinAddressHex = web3.utils.asciiToHex(data.bitcoinAddress).slice(2);
            ...
            const amountInSatoshis = (parseFloat(data.amount) * 100000000).toString();

            console.log('Bitcoin Address (Hex):', bitcoinAddressHex);
            console.log('Amount (Satoshis):', amountInSatoshis);
            console.log('Expiration Time:', Math.floor(Date.now() / 1000) + expiryInSeconds);

            const tx = await contract.methods.makeRequest(
                bitcoinAddressHex,
                amountInSatoshis,
    ...
    const validatePositiveInteger = (value: string): string | boolean => {
        ...
        return Number.isInteger(number) && number >= 0 || 'Must be a positive integer';
    };
                                ...
                                <div className="flex space-x-2">
                                    <div className="flex items-center">
                                        ...
                                        { errors.expirationDays &&
                                            <span className="text-red-500">{ errors.expirationDays.message }</span> }
                                    </div>
                                    <div className="flex items-center">
                                        ...
                                        { errors.expirationHours &&
                                            <span className="text-red-500">{ errors.expirationHours.message }</span> }
                                    </div>
                                </div>
                            </div>
                        ...
                        <div>
                            <div className="mb-4">
                                <div className="mb-2 text-gray-700">
                                    <ul>
                                        <li>
                                            <b>Custody:</b> Your Bitcoin will always remain under your control until you
                                            decide to make
                                            a transaction.
                                        </li>
                                        <li className="mt-3">
                                            <b>Transparency:</b> The Bitcoin address you provide will be publicly
                                            visible, allowing
                                            users to verify it on the blockchain Explorer.
                                        </li>
                                        <li className="mt-3">
                                            <b>Monitor Bids:</b> Regularly check the platform for new bids. When you
                                            find an

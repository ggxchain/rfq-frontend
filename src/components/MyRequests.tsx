...
const MyRequests: React.FC = () => {
    ...
    const fetchRequests = async () => {
        ...
        try {
            ...
            const requestCounterStr: string = await contract.methods.requestCounter(web3.utils.asciiToHex(account)).call();
            const requestCounter = parseInt(requestCounterStr, 10);
            ...
            for (let i = 0; i < requestCounter; i++) {
                ...
                } = await contract.methods.requests(web3.utils.asciiToHex(account), i).call();

                fetchedRequests.push({
                    id: i,
                    expiry: Number(request.expiry),
                    btcAmount: parseFloat(web3.utils.fromWei(request.amount, 'gwei')), // Using 'gwei' for BTC to satoshi conversion
                ...
                {requests.map((data) => (
                    <tr key={data.id}>
                        ...
                        <td className="font-mono">
                            {data.btcAddress}
                            <button className={styles.iconButton} onClick={() => copyToClipboard(data.btcAddress)}>
                                <FontAwesomeIcon icon={faCopy} />
                            </button>
                            <button className={styles.iconButton} onClick={() => openInExplorer(data.btcAddress)}>
